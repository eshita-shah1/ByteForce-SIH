"""Loads the RETIRED v2 Model 2 pipeline from models/ and runs end-to-end
inference (no mocks). Kept only to verify the old artifact still loads and
predicts correctly for the migration comparison
(test_model2_migration_comparison.py) - this service is not on any live
request path. See app/services/model2_service_v2_legacy.py."""
import pytest
...
def loaded_service():
    ...
    if not settings.model2_pipeline_path.exists() and not settings.model2_pipeline_fallback_path.exists():
        pytest.skip("Legacy Model 2 artifacts not present in models/")
    svc = LegacyModel2Service()
    svc.load(settings)
    if not svc.is_loaded:
        pytest.skip("Legacy Model 2 failed to load in this environment (see README version-compatibility notes).")
    return svc
...
def test_legacy_model2_loads(loaded_service):
    assert loaded_service.is_loaded


def test_legacy_model2_predict_returns_float(loaded_service):
    prediction = loaded_service.predict(_dummy_input())
    assert isinstance(prediction, float)


def test_legacy_model2_predict_raises_on_missing_feature(loaded_service):
