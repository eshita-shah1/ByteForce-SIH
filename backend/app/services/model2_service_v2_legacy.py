"""Model 2 (production shortfall) inference service - RETIRED "v2" artifact.

2026-09-22: superseded as the live model by app/services/model2_service.py
(the "v3" deployment package - bare XGBRegressor + SHAP explainer, 15
numeric features, no pit_id/shift_type/month/day_of_week). This module is
kept byte-for-byte identical to the pre-replacement model2_service.py
ONLY so backend/tests/test_model2_migration_comparison.py can still load
and run the old artifact side-by-side with the new one. It is NOT imported
by app/main.py or any live request path - not loaded at app startup, not
wired into predict_shortfall(). Do not delete MOIL_Module2_Final_Model.pkl/
moil_production_pipeline.pkl (or this module) until the v3 migration is
explicitly approved (see shortfall_service.py's migration comparison notes).
...
legacy_model2_service = LegacyModel2Service()
